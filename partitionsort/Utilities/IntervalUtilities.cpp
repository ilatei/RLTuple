...
std::vector<PSRule> Utilities::RedundancyRemoval(const std::vector<PSRule>& rules) {
	std::vector<bool> isVisited(rules.size(), 0);
	std::vector<PSRule> out;
...
bool  Utilities::IsIdentical(const PSRule& r1, const PSRule& r2) {
...
std::pair<std::vector<int>, int> Utilities::MWISIntervals(const std::vector<PSRule>& I, int x) {
	// create an weighted interval from rules
	std::vector<WeightedInterval> vc;
	for_each(begin(I), end(I), [&vc, x](const PSRule & r) { vc.push_back(WeightedInterval(std::vector<PSRule>(1, r), x)); });
...
std::vector<WeightedInterval> Utilities::CreateUniqueInterval(const std::vector<PSRule>& rules, int field) {
