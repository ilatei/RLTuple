...
void HybridTSS::ConstructClassifier(const vector<PSRule> &rules) {
//    train()
...
int HybridTSS::ClassifyAPacket(const PSPacket &packet) {
    return root->ClassifyAPacket(packet);
//    return 0;
}

void HybridTSS::DeleteRule(const PSRule &rule) {
    root->DeleteRule(rule);
}

void HybridTSS::InsertRule(const PSRule &rule) {
...
vector<int> HybridTSS::getAction(SubHybridTSS *state, int epsilion = 100) {
    ...
    vector<PSRule> nodeRules = state->getRules();
    if (nodeRules.size() < binth) {
        return {linear, -1, -1};
    }
    set<uint32_t> tupleKey;
    for (const PSRule &r : nodeRules) {
...
void HybridTSS::ConstructBaseline(const vector<PSRule> &rules) {
    root = new SubHybridTSS(rules);
    root->nodeId = 0;
    queue<SubHybridTSS*> que;
    que.push(root);
...
void HybridTSS::train(const vector<PSRule> &rules) {
...
int HybridTSS::ClassifyAPacketAccess(const PSPacket& one_packet, ProgramState *program_state){
    return 0;
}

int HybridTSS::CalculateState(ProgramState *program_state) {
	return 0;
}
