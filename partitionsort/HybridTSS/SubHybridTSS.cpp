...
SubHybridTSS::SubHybridTSS(const vector<PSRule> &r) {
    this->rules = r;
//    offsetBit = {0, 32, 0, 32, 0, 16, 0, 16};
    state = 0;
    reward = 0;
...
SubHybridTSS::SubHybridTSS(const vector<PSRule> &r, vector<int> offsetBit) {
    this->rules = r;
    reward = -1;
...
SubHybridTSS::SubHybridTSS(const vector<PSRule> &r, int s, SubHybridTSS* p) {
    this->rules = r;
    this->state = s;
    bigClassifier = nullptr;
    maxBigPriority = -1;
    ...
    switch(fun) {
        ...
        case TM: {
            ...
            if (mode == "build") {
                ...
                this->TMO = new TupleMergeOnline(args);
...
//            bit = 9 + op[2];
            ...
            vector<vector<PSRule> > subRules(nHashTable + 1);
...
int SubHybridTSS::ClassifyAPacket(const PSPacket &packet) {
    int matchPri = -1;
    switch (fun) {
        case linear: {
...
bool cmp(const PSRule &rule1, const PSRule &rule2){
    return rule1.id > rule2.id;
}

void SubHybridTSS::DeleteRule(const PSRule &rule) {
    switch (fun) {
        case linear: {
            auto iter = std::lower_bound(rules.begin(), rules.end(), rule, cmp);
            if (iter == rules.end()) {
                std::cout << "Not found rule" << rule.id << std::endl;
                return;
            }
            rules.erase(iter);
            return ;
...
void SubHybridTSS::InsertRule(const PSRule &rule) {
    switch (fun) {
        case linear: {
            auto iter = lower_bound(rules.begin(), rules.end(), rule, cmp);
...
uint32_t SubHybridTSS::getKey(const PSRule &r) const {
    uint32_t Key = 0;
...
uint32_t SubHybridTSS::getKey(const PSPacket &p) const {
    uint32_t Key = 0;
...
uint32_t SubHybridTSS::getRulePrefixKey(const PSRule &r) {
...
vector<PSRule> SubHybridTSS::getRules() {
    return rules;
}

void SubHybridTSS::FindRule(const PSRule &rule) {
...
void SubHybridTSS::FindPacket(const PSPacket &packet) {
    int matchPri = -1;
    cout << nodeId << " ";
    switch (fun) {
        case linear: {
