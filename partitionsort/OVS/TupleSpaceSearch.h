...
struct TSSTuple {
public:
	TSSTuple(const std::vector<int>& dims, const std::vector<unsigned int>& lengths, const PSRule& r) : dims(dims), lengths(lengths) {
		for (int w : lengths) {
			tuple.push_back(w);
		}
	...
	Memory MemSizeBytes(Memory ruleSizeBytes) const {
		uint64_t memory_size = sizeof(TSSTuple);
...
struct PriorityTuple : public TSSTuple {
public:
	PriorityTuple(const std::vector<int>& dims, const std::vector<unsigned int>& lengths, const PSRule& r) :TSSTuple(dims, lengths, r){
...
public:
	...
	void PlotTupleDistribution() {
		...
		std::vector<std::pair<unsigned int, TSSTuple> > v;
		copy(all_tuples.begin(), all_tuples.end(), back_inserter(v));
		
		auto cmp = [](const std::pair<unsigned int, TSSTuple>& lhs, const std::pair<unsigned int, TSSTuple>& rhs) {
...
protected:
	...
	std::unordered_map<uint64_t, TSSTuple> all_tuples;
