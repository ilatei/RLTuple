...
void TSSTuple::Insertion(const PSRule& r) {
...
void TSSTuple::Deletion(const PSRule& r) {
...
int TSSTuple::WorstAccesses() const {
	// TODO
	return 1;//cmap_largest_chain(&map_in_tuple);
}

int TSSTuple::FindMatchPacket(const PSPacket& p)  {
...
int TSSTuple::FindMatchPacketAccess(const PSPacket& p, ProgramState *program_state)  {
...
bool inline TSSTuple::IsPacketMatchToRule(const PSPacket& p, const PSRule& r) {
...
uint32_t inline TSSTuple::HashRule(const PSRule& r) const {
	uint32_t hash = 0;
...
uint32_t inline TSSTuple::HashPacket(const PSPacket& p) const {
	uint32_t hash = 0;
...
int TupleSpaceSearch::ClassifyAPacketAccess(const PSPacket& packet, ProgramState *program_state) {
	...
	int query = 0;
	for (auto& tuple : all_tuples) {
		auto result = tuple.second.FindMatchPacketAccess(packet, program_state);
		priority = std::max(priority, result);
		query++;
	}
	QueryUpdate(query);
...
void TupleSpaceSearch::InsertRule(const PSRule& rule) {
	...
	if (hit != end(all_tuples)) {
		...
		all_tuples.insert(std::make_pair(KeyRulePrefix(rule), TSSTuple(dims, lengths, rule)));
	}
