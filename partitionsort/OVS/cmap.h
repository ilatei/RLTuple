...
struct cmap_node {
	...
	cmap_node(const PSRule& r) : priority(r.priority), rule_ptr(std::make_shared<PSRule>(r)), next(nullptr){ }
	unsigned int key;
	int priority;
	std::shared_ptr<PSRule> rule_ptr;
...
uint64_t CmapMemorySize(const struct cmap *cmap);
