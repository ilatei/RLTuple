...
int RBExactQueryPriorityAccess(rb_red_blk_tree*  tree, const PSPacket& q, int level, const std::vector<int>& fieldOrder, int priority_so_far, ProgramState *program_state); 
...
void RBSerializeIntoRulesRecursion(rb_red_blk_tree * tree, rb_red_blk_node* node, int level, const std::vector<int>& fieldOrder, std::vector<box>& boxes_so_far, std::vector<PSRule>& rules_so_far);
