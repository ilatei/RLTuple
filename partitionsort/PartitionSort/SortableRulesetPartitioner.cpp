...
std::pair<bool, std::vector<int>> SortableRulesetPartitioner::GreedyFieldSelectionTwoIterations(const std::vector<PSRule>& rules) {
	if (rules.size() == 0) {
	...
	std::vector<PSRule> current_rules;
...
std::pair<std::vector<PSRule>, std::vector<int>> SortableRulesetPartitioner::GreedyFieldSelection(const std::vector<PSRule>& rules)
{
	if (rules.size() == 0) {
		printf("Warning: GreedyFieldSelection rule size = 0\n ");
	}
...
std::vector<SortableRuleset> SortableRulesetPartitioner::SortableRulesetPartitioningGFS(const std::vector<PSRule>& rules ) {
	std::vector<PSRule> current_rules = rules;
...
std::vector<SortableRuleset> SortableRulesetPartitioner::AdaptiveIncrementalInsertion(const std::vector<PSRule>& rules, int threshold) {
	std::vector<SortableRuleset> all_buckets;
	
	auto InsertRuleIntoAllBucket = [threshold](const PSRule& r, std::vector<SortableRuleset>& b) {
		for (auto& bucket : b) {
...
std::vector<int> SortableRulesetPartitioner::GetFieldOrderByRule(const PSRule& r)
 {
	 ...
	 for (int i = 0; i < r.dim; i++) {
		 int imodn = i % r.dim;
		 if (imodn == 0 || imodn == 1 || imodn == 5) {
			 //IP, in_port
			 unsigned int length = r.range[imodn][HIGH] - r.range[imodn][LOW] + 1;
			 if (length == 1) rank[i] = 0;
			 else if (length > 0 && length < (1 << 31)) rank[i] = 1;
			 else if (length == 0) rank[i] = 3;
			 else rank[i] = 2;
		 } else if (imodn == 2 || imodn == 3 || imodn == 8) {
			 //Port, eth_type
			 unsigned int length = r.range[imodn][HIGH] - r.range[imodn][LOW] + 1;
			 if (length == 1) rank[i] = 0;
		 ...
		 else if(imodn == 6 || imodn == 7){
			// dl_mac
			uint64_t length = r.range[imodn][HIGH] - r.range[imodn][LOW] + 1;
			if (length == 1) rank[i] = 0;
			 else if (length < ((uint64_t)1 << 47)) rank[i] = 1;
			 else if (length < ((uint64_t)1 << 48)) rank[i] = 2;
			 else rank[i] = 3;
		 }
		 else if (imodn == 9){
			 //Protocol
			 unsigned int length = r.range[imodn][HIGH] - r.range[imodn][LOW] + 1;
			 if (length == 1) rank[i] = 0;
			 else if (length < (1 << 11)) rank[i] = 1;
			 else if (length < (1 << 12)) rank[i] = 2;
			 else rank[i] = 3;
		 }
		 else if (imodn == 10){
			 //dl_vlan_pcp
			 unsigned int length = r.range[imodn][HIGH] - r.range[imodn][LOW] + 1;
			 if (length == 1) rank[i] = 0;
			 else if (length < (1 << 2)) rank[i] = 1;
			 else if (length < 8) rank[i] = 2;
			 else rank[i] = 3;
		 }
	 }
...
int SortableRulesetPartitioner::ComputeMaxIntersection(const std::vector<PSRule>& ruleset){
...
int SortableRulesetPartitioner::ComputeMaxIntersectionRecursive(const std::vector<PSRule>& rules, int field_depth, const std::vector<int>& field_order){

	if (rules.size() == 0) return 0;
	if (field_depth <= 0) {
		std::multiset<unsigned int> Astart;
		std::multiset<unsigned int> Aend;
		std::vector<PSRule> rules_rr = Utilities::RedundancyRemoval(rules);
		for (auto v : rules_rr){
	...
	int i_e = 0;
	int max_POM = 0;
	std::vector<PSRule> rule_this_loop;
...
std::vector<SortableRuleset>  SortableRulesetPartitioner::MaximumIndepdenentSetPartitioning(const std::vector<PSRule>& rules) {
	std::vector<PSRule> current_rules = rules;
	std::vector<SortableRuleset> all_buckets;
	int sum_rank = 0;
	while (!current_rules.empty()) {
		...
		std::vector<PSRule> temp_rules = out.first;
...
std::pair<std::vector<PSRule>, std::vector<int>> SortableRulesetPartitioner::MaximumIndependentSetAllFields(const std::vector<PSRule>& rules){
	std::vector<int> all_fields(rules[0].dim);
	std::iota(begin(all_fields), end(all_fields), 0);
	std::vector<PSRule> best_so_far_rules(0);
...
std::vector<PSRule> SortableRulesetPartitioner::MaximumIndependentSetGivenField(const std::vector<PSRule>& rules, const std::vector<int>& fields){
		return MaximumIndependentSetGivenFieldRecursion(rules, fields, 0, 0, 0).GetRules();
}

WeightedInterval SortableRulesetPartitioner::MaximumIndependentSetGivenFieldRecursion(const std::vector<PSRule>& rules, const std::vector<int>& fields, int depth, unsigned a, unsigned b){
	if (a > b) {
...
std::pair<bool, std::vector<int>> SortableRulesetPartitioner::FastGreedyFieldSelectionTwoIterations(const std::vector<PSRule>& rules) {
	if (rules.size() == 0) {
		printf("Warning: GreedyFieldSelection rule size = 0\n ");
	}
	int num_fields = rules[0].dim;
...
std::pair<std::vector<PSRule>, std::vector<int>> SortableRulesetPartitioner::FastGreedyFieldSelection(const std::vector<PSRule>& rules)
{
	if (rules.size() == 0) {
		printf("Warning: GreedyFieldSelection rule size = 0\n ");
	}
...
std::pair<bool, std::vector<int>> SortableRulesetPartitioner::FastGreedyFieldSelectionForAdaptive(const std::vector<PSRule>& rules) {
	if (rules.size() == 0) {
		printf("Warning: GreedyFieldSelection rule size = 0\n ");
	}
	int num_fields = rules[0].dim;
