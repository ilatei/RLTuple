...
public:
	...
	OptimizedMITree(const PSRule& r) {
		root = RBTreeCreate();
		numRules = 0; 
	...
	void Insertion(const PSRule& rule) {
	...
	void Insertion(const PSRule& rule, bool& priorityChange) {
   ...
   //		} 
	...
	bool TryInsertion(const PSRule& rule, bool& priorityChange) {
		if (CanInsertRule(rule)) {
			counter++;
			priorityContainer.insert(rule.priority);
			priorityChange = rule.priority > maxPriority;
	...
	void Deletion(const PSRule& rule, bool& priorityChange) {
	...
	bool CanInsertRule(const PSRule& r) const {
	...
	int ClassifyAPacket(const PSPacket& one_packet)const {
		return  RBExactQueryIterative(root, one_packet,  fieldOrder);
		//	return   RBExactQuery(root, one_packet, 0,fieldOrder);
	}
	int ClassifyAPacketAccess(const PSPacket& one_packet, ProgramState *program_state)const {
		return  RBExactQueryIterativeAccess(root, one_packet,  fieldOrder, program_state);
		//	return   RBExactQuery(root, one_packet, 0,fieldOrder);
	}
	int ClassifyAPacket(const PSPacket& one_packet,int priority_so_far)const {
		return   RBExactQueryPriority(root, one_packet, 0, fieldOrder, priority_so_far);
	}
	int ClassifyAPacket(const PSPacket& one_packet,int priority_so_far, ProgramState *program_state)const {
		return   RBExactQueryPriorityAccess(root, one_packet, 0, fieldOrder, priority_so_far, program_state);
	}
	...
	void ReconstructIfNumRulesLessThanOrEqualTo(int threshold = 10) {
		...
		//global_counter++;
		std::vector<PSRule> serialized_rules = SerializeIntoRules(); 
	...
	std::vector<PSRule> SerializeIntoRules() const {
